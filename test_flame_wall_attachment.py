...
def create_initial_flame_profile_from_line(X, Y, x0, y0, x1, y1, thickness=0.05):
    ...
    The flame is represented as:
    G < 0: unburnt (below/left of line)
    G > 0: burnt (above/right of line)
    G = 0: flame surface (the line itself)
    ...
    x0, y0 : float
        Start point of flame line (anchor point, typically 0, 0)
    x1, y1 : float
        End point of flame line
    thickness : float
        Transition thickness for smooth profile (default: 0.05)
    ...
    tx = dx / line_length  # tangent x
    ty = dy / line_length  # tangent y
    
    # Normal vector (perpendicular to line, pointing to burnt side)
    # For a line going from (x0,y0) to (x1,y1), the perpendicular pointing "up/left" is:
    nx = -ty
    ny = tx
    
    # Signed distance from the line
    # For a point (x, y), the signed distance to the line through (x0, y0) with normal (nx, ny) is:
    # d = (x - x0) * nx + (y - y0) * ny
    distance = (X - x0) * nx + (Y - y0) * ny
    
    # Create smooth transition using tanh
    # Positive distance = burnt, negative = unburnt
    G = np.tanh(distance / thickness)
    
    # We need to limit the flame to only exist between the anchor and the tip
    # Beyond the tip (x1, y1), everything should be unburnt
    
    # For each point, check if it's beyond the flame tip
    # Project point onto line direction to see if it's past the tip
    for j in range(Y.shape[0]):
        for i in range(X.shape[1]):
            ...
            # If projection is beyond line length, we're past the tip
            if proj > line_length:
                # Force to unburnt
                G[j, i] = -1.0
            elif proj < 0:
                # Before the anchor (shouldn't happen at x=0, but for safety)
                G[j, i] = -1.0
